...
if flock -xn 200; then
	...
	if [[ ! -f "$basesegpath".webm ]]; then
		...
		ffmpeg -ss "$ss" -i "$infile" -vf "$vf" -t "$t" -copyts -start_at_zero -vsync passthrough -enc_time_base -1 -an -sn -map_metadata -1 $codecflags -g 1000 -keyint_min 1000 -dash 1 -dash_segment_type webm -init_seg_name "$basesegname"_init.webm -media_seg_name "$basesegname".webm "$basesegpath".mpd
		rm "$basesegpath".mpd
